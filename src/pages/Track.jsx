...
import { C, grad, trackGrad } from '../theme'
...
export default function Track() {
  ...
  return (
    <main style={{ background: C.ink, color: '#fff', minHeight: '100vh' }}>
      <section style={{ position: 'relative', overflow: 'hidden', background: isAi ? grad.ai : 'linear-gradient(160deg,#16224A,#0A0B0D)' }}>
        <img src={`/assets/${isAi ? 'orn-flower' : 'orn-rings'}.svg`} alt="" style={{ position: 'absolute', right: '-4%', top: '10%', width: 'clamp(220px,30vw,420px)', opacity: 0.18 }} />
        <div style={{ position: 'relative', maxWidth: 1480, margin: '0 auto', padding: 'clamp(48px,7vw,108px) clamp(20px,5vw,56px) clamp(40px,6vw,80px)' }}>
          ...
          <Eyebrow color={isAi ? '#FFB37A' : '#8FB4FF'}>{tr.en.toUpperCase()} TRACK</Eyebrow>
          ...
          <Link to={`/lesson/${chs[0].id}`} className="cta-pill" style={{ display: 'inline-flex', marginTop: 34, padding: '15px 30px', borderRadius: 60, background: isAi ? 'linear-gradient(135deg,#FF7A1E,#E0470A)' : grad.blue, color: '#fff', fontSize: 15.5, fontWeight: 700, alignItems: 'center', gap: 8 }}>
            1강부터 시작하기 →
          </Link>
        </div>
      </section>
